...
# (see http://www.python.org/dev/peps/pep-3148/#module-functions)
...
def _startup(rootFuture, *args, **kargs):
    ...
    :param rootFuture: Any callable object (function or class object with
        *__call__* method); this object will be called once and allows the
        use of parallel calls inside this object.
...
def _mapFuture(callable, *iterables, **kargs):
    ...
    :param callable: Any callable object (function or class object with
        *__call__* method); this object will be called to execute each Future.
...
def map(func, *iterables, **kargs):
    ...
    done in future version of SCOOP].
...
def submit(func, *args, **kargs):
    ...
    Result retrieval is made via the ``result()`` function on the Future."""
    try:
...
def wait(fs, timeout=None, return_when=ALL_COMPLETED):
    ...
    :param fs: The sequence of Futures (possibly created by another instance)
        to wait upon.
    :param timeout: The maximum number of seconds to wait [To be done in future
    ...
    :return: A named 2-tuple of sets. The first set, named 'done', contains the
        ...
        futures."""
    DoneAndNotDoneFutures = namedtuple('DoneAndNotDoneFutures',
                                       'done not_done')
